...
function mdtTableDirective(tableDataStorageServiceFactory:TableDataStorageServiceFactory,
    ...
    return {
        ...
        controller: function mdtTableController($scope:IScope) {
            ...
            function initTableStorageServiceAndBindMethods() {
                ...
                if (!$scope['mdtRowPaginator']) {
                    $scope['mdtPaginationHelper'] = mdtPaginationHelperFactory
                        .getInstance(vm.tableDataStorageService, $scope['paginatedRows']);
                }else {
            ...
            function addHeaderCell(ops:any) {
                vm.tableDataStorageService.addHeaderCellData(ops);
            }
        },
        link: function($scope:IScope, element:any, attrs:any, ctrl:any, transclude:any) {
            ...
            function processAttributeProvidedData() {
                ...
                if (angular.isUndefined(attrs.mdtRowPaginator)) {
                    $scope.$watch('mdtRow', function (mdtRow:any) {
            ...
            function addRawDataToStorage(data:any) {
                var rowId:any;
                var columnValues:Array<any> = [];
                _.each(data, function(row:RowData){
            ...
            function isPaginationEnabled() {
                if ($scope['paginatedRows'] === true
                    || ($scope['paginatedRows']
                    && $scope['paginatedRows'].hasOwnProperty('isEnabled')
                    && $scope['paginatedRows'].isEnabled === true)) {
                    return true;
                }
            ...
            function injectContentIntoTemplate() {
                transclude(function (clone:any) {
                    var headings:Array<any> = [];
                    var body:Array<any> = [];
                    var customCell:Array<any> = [];

                    _.each(clone, function (child:any) {
            ...
            function showEditDialog(ev:any, cellData:any, rowData:any) {
                ...
                var that = this;
                $mdDialog.show(ops).then(function(cellValue:any) {
